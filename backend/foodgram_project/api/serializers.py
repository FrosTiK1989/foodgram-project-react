...
class RecipeGetSerializer(serializers.ModelSerializer):
    ...
    def get_ingredients(self, recipe):
        return AmountIngredientForRecipeGetSerializer(
            AmountIngredientForRecipe.objects.filter(recipe=recipe), many=True
        ).data
...
class FavoriteRecipesSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if FavoriteRecipe.objects.filter(
            user=self.context.get("request").user, recipe=data["recipe"]
        ).exists():
            raise serializers.ValidationError({"status": "Уже добавлен"})
        return data
